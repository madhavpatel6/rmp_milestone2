...
def Calc_keys(s):
	k1=min([g[s],rhs[s]])+heuristic(s,start)+Km
	k2=min([g[s],rhs[s]])
	key=[k1,k2]
	#print("calc kyes: ",key)
	return key
...
def Calc_shortest_path():
	counter=0
	print(U.queue[0][0],start,rhs[start],float(rhs[start]),float(g[start]),float(rhs[start]) != float(g[start]),rhs[start] is not g[start])
	while U.queue[0][0] < Calc_keys(start)[0] or float(rhs[start]) != float(g[start]):
		#print(U.queue[0][0],start,g[goal+1],float(rhs[start]),float(g[start]),float(rhs[start]) != float(g[start]),rhs[start] is not g[start])
		KU=U.get()
		k_old=KU[0]
		u=KU[1]
